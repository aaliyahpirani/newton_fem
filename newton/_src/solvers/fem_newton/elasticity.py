...
def nh_hessian_proj(F: wp.mat33, tau: wp.mat33, sig: wp.mat33, lame: wp.vec2):
    ...
    mu_nh, lambda_nh = nh_parameters_from_lame(lame)
    dpsi_dpsi = mu_nh * wp.ddot(tau, sig) + lambda_nh * wp.ddot(dJ_dF_s, tau) * wp.ddot(dJ_dF_s, sig)
...
def snh_energy(F: wp.mat33, lame: wp.vec2):
    ...
    return 0.5 * (lambda_nh * (J - gamma) * (J - gamma) + mu_nh * (Ic - wp.log(Ic + 1.0)) - E0)
...
def hessian_proj_analytic(
    ...
    A = muT * wp.identity(n=3, dtype=float) + muL * wp.outer(S, S) + lbd * wp.outer(Scross, Scross) + lbdJ * Soff
